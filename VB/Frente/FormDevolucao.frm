...
If MsgBox("Confirma devolução ?", vbYesNo, App.Title) = vbYes Then
    ...
    Set DatDev = FrmFrente.Banco.OpenRecordset("SELECT * FROM ITP WHERE Pedido = " & DBGrid.Columns(0))
    DatDev.Requery
    
    Set DatPed = FrmFrente.Banco.OpenRecordset("SELECT * FROM ITP WHERE Pedido = " & VarPedDec)
    
    sql = "DELETE DUP.*, DUP.Pedido FROM DUP WHERE DUP.Pedido = " & DBGrid.Columns(0)
...
DatDup.DatabaseName = FrmFrente.Caminho & "\Dados.mdb"

DatPed.RecordSource = ("SELECT PED.CodPed, PED.DataEmiss, ENT.Nome, PED.Total, PED.Operacao, PED.Entidade FROM ENT INNER JOIN PED ON ENT.CodEntidade = PED.Entidade WHERE PED.Entidade = " & VarCodDev & " ORDER BY CodPed")
DatPed.Refresh
