...
If DatDados(0).Recordset.RecordCount > 0 Then
    ...
    Set Itens = FrmFrente.Banco.OpenRecordset("ITP")
    While Not DatDados(0).Recordset.EOF
    
'        Criterio = "CodPrd = " & VarVenda!Produto
'        Registros.FindFirst Criterio
'        FrmFrente.TxtTmp = DatDados(0).Recordset("Valor")
'        FrmFrente.TxtDados(0).Text = DatDados(0).Recordset("Qtd")
'        FrmFrente.TxtDados(0).SetFocus
'        FrmFrente.TxtDados_KeyPress 0, 13

'        FrmFrente.TxtDados(2).Text = DatDados(0).Recordset("CodPrd")
'        FrmFrente.TxtDados(2).SetFocus
'        FrmFrente.TxtDados_KeyPress 2, 13
...
'    Wend
        ...
        If DatDados(0).Recordset("Qtd") <> 0 Then
            Itens.AddNew
            Itens("Pedido") = FrmFrente.LblPedido
...
'                 BaixarEstoque .DatCompra.Recordset.Fields("Qtd"), .DatCompra.Recordset.Fields("Produto"), .DatCompra.Recordset.Fields("Loja"), True
 ...
 '               .LstPesq.SubItems(3) = .DatCompra.Recordset.Fields("CodBar")
                ...
                .LstPesq.SubItems(5) = Numero(Format(.DatCompra.Recordset("ValorUnit"), " ##,##0.000"), 3)  'SeCampoNulo(Consulta, "Conta")
                .LstPesq.SubItems(6) = Numero(Format(CCur(.DatCompra.Recordset("Qtd")), "##,##0.000") * Format(CCur(.DatCompra.Recordset("ValorUnit")), "##,##0.000"), 3)  'SeCampoNulo(Consulta, "DataOper")
...
For Each Componentes In DatDados
    Componentes.DatabaseName = LocalBanco & "\Dados.mdb"
Next
