...
Begin VB.Form FrmRelVdaFunCid 
   ...
   End
End
Attribute VB_Name = "FrmRelVdaFunCid"
...
Private Sub Relatorio_ImprimiuRegistro(Cancelar As Boolean)
    If Not IsNull(Relatorio.Recordset("Cidade")) Then
        VarDesc = Relatorio.Recordset("Cidade")
    End If
'    VarCodGru = Relatorio.Recordset("CodGru")
    VarContador = Relatorio.Recordset("ContarDeCodPed")
    VarQtd = Relatorio.Recordset("SomadeQTd")
...
Private Sub Relatorio_FormulaGrupo(ByVal Ordem As Byte, Valor As Variant)
    ...
    If Ordem = 1 Then
        Valor = Relatorio.Recordset("PrimeiroDeCodEntidade")
    End If
    If Ordem = 2 Then
